...
const createMenu = (menu) => {
  ...
  let resultado = {
    fetchMenu: () => menu,
    consumption: Object.keys(menu.food).concat(Object.keys(menu.drinks)),
    order: (string) => {
      resultado.consumption.push(string);
      return resultado;
    },
    pay: () => {
      let soma = 0;
      resultado = Object.values(menu.food).concat(
        Object.values(menu.drinks),
      );
...
meuRestaurante.order('pao de queijo');

console.log(
  createMenu({
    food: { coxinha: 5, pastel: 5 },
    drinks: { agua: 5, refri: 5 },
  }).pay(),
);
